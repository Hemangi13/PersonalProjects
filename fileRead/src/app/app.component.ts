...
export class AppComponent  {
      ...
      data => {
        ...
        for (let index = 1; index < this.arrayForWidths.length + 1; index++) 
        {
          ...
          for(let i = 1; i < this.arrayForWidths.length + 1; i++)
          {
            ...
            let dataChunk = prnToRowArray[index].substring(chunkStart, chunkEnd).trimEnd().trimStart();
  ...
  let csvimputfile = 'assets/Workbook2.csv'; */
  //const result1 = fs.readFileSync('assets/Workbook2.csv', {encoding: 'utf-8'});
  //console.log(result1);
 /*  let csvOutputFile = 'Output.JSON'
  let result = csvToJson.generateJsonFileFromCsv(csvimputfile,csvOutputFile);
  console.log(result); */
